...
`.mjs` file copied from the locally staged post-`v0.5.0` release asset
candidate and generates a product-local `dist/js/ms-office-core.js` adapter
during `npm run build`. That adapter transforms the vendored ESM into an IIFE
global, registers it in `getDocx2mdModuleRegistry()` as `msOfficeCore`, and
lets `zip-io.ts` call `readZipPackageAsync`.
...
This is the locally staged patch-suffix candidate generated by
`TAG_NAME=v0.5.0.1 npm run prepare:release-assets`. As of 2026-06-22, the
candidate is not yet published on GitHub Release.
...
- Publish or consume a post-`v0.5.0` release asset that avoids top-level Node
  builtin imports before testing browser-oriented product builds.
